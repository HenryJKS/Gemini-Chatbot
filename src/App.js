    ...
    <div className="principal d-flex justify-content-center align-items-center" style={{ height: "100vh" }}>
      <Card className="w-50" style={{ background: 'rgba(255, 255, 255, 0.15)', borderRadius: '16px', boxShadow: '0 4px 30px rgba(0, 0, 0, 0.1)', backdropFilter: 'blur(2px)', borderColor: 'black', WebkitBackdropFilter: 'blur(3.8px)' }}>
        ...
        <Card.Body>
          ...
          <button
            onClick={getResponse}
            disabled={!text || loading} // Desabilite o botão se não houver texto ou se estiver carregando
            className="mt-3"
            style={{ margin: "auto", justifyContent: "center", justifyItems: "center", display: "flex" }}>
            {loading ? (<Spinner as="span" animation="border" size="sm" role="status" aria-hidden="true" />) : (<span>➛ Send</span>)}
          </button>
        </Card.Body>
      </Card>
    </div>
